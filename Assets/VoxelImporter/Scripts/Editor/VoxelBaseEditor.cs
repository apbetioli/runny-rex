    ...
    public abstract class VoxelBaseEditor : EditorCommon
    {
        ...
        protected void UpdateMaterialList()
        {
            ...
            materialList.drawHeaderCallback = (rect) =>
            {
                ...
                r.x = r.width - 128;
                if (baseTarget.voxelData != null && baseTarget.voxelData.materials != null)
                {
                    EditorGUI.BeginChangeCheck();
                    var loadFromVoxelFile = EditorGUI.ToggleLeft(r, "Load From Voxel File", baseTarget.loadFromVoxelFile);
                    if (EditorGUI.EndChangeCheck())
                    {
                        UndoRecordObject("Inspector");
